...
if [ -e ${BLOCK_DEVICE} ]; then
    # userdata
    if [ "${BLOCK_DEVICE}" == "/dev/block/mmcblk0p22" ];then
        if [ "${SYSPART}" == "system" ];then
            MOUNT_POINT="/system"
    elif [ "${BLOCK_DEVICE}" == "/dev/block/mmcblk0p25" ];then
        if [ "${SYSPART}" == "system2" ];then
            MOUNT_POINT="/.secondrom/"
        else
            reboot recovery
        fi
...
RECOVERY_NODE="$(busybox readlink -f /dev/block/mmcblk0p21)"
